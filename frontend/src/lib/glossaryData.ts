...
export const glossaryEntries: GlossaryEntry[] = [
  ...
  {
    term: 'simple-patterns',
    displayName: 'Simple Patterns',
    shortDefinition: 'Single-source, high-confidence patterns with a clear, linear decision path — straightforward segments where the agent can confidently predict outcomes with minimal cross-referencing.',
    fullDefinition: 'Simple patterns are the clearest signals in your data. They represent segments where outcomes are determined by a single primary feature or a straightforward linear combination of features, with little ambiguity or need for inference. The agent can identify these patterns immediately, apply a direct rule, and move forward with high confidence. These patterns are the fastest for the agent to process and the most reliable in production.',
    whyItMatters: 'Simple patterns are the agent\'s fastest, most confident decisions. They drive the majority of automated responses. A well-populated set of simple patterns means the agent can handle most incoming items directly without requiring human oversight.',
    category: 'pattern',
    relatedTerms: ['complex-patterns', 'fuzzy-patterns', 'sufficient-data', 'cohort'],
    stages: [4, 5],
  },
  {
    term: 'complex-patterns',
    displayName: 'Complex Patterns',
    shortDefinition: 'Multi-source patterns requiring inference and cross-referencing — segments where the agent must synthesise information from multiple features to confidently route or classify.',
    fullDefinition: 'Complex patterns emerge when outcomes depend on the interaction of multiple features or when differentiating between cases requires combining signals from several sources. The agent must examine multiple data points, weigh their relative importance, and often cross-reference with external context or knowledge bases to make a confident decision. These patterns are more compute-intensive and benefit from retrieval-augmented processing or human-in-the-loop validation, but when handled correctly they unlock high-accuracy handling of nuanced cases.',
    whyItMatters: 'Complex patterns are where the agent\'s real power emerges — handling the intricate, multi-faceted cases that simpler decision rules cannot address. Mastering complex pattern routing is the difference between an agent that handles simple cases and one that truly solves complex problems.',
    category: 'pattern',
    relatedTerms: ['simple-patterns', 'fuzzy-patterns', 'sufficient-data'],
    stages: [4, 5],
  },
  {
    term: 'fuzzy-patterns',
    ...
    whyItMatters: 'These segments are the highest-value opportunity for a domain expert. Adding one well-chosen parameter — a behavioral signal, an external data source, or a derived feature — can often convert Fuzzy Patterns into Simple or Complex ones, directly improving prediction accuracy.',
    category: 'pattern',
    relatedTerms: ['simple-patterns', 'complex-patterns', 'confidence'],
    stages: [4, 5],
  },
  {
    term: 'cohort',
    ...
    category: 'pattern',
    relatedTerms: ['simple-patterns', 'sufficient-data'],
    stages: [4, 5],
  },
  {
    term: 'confidence',
  ...
  },
  {
    ...
    category: 'pattern',
    relatedTerms: ['simple-patterns', 'complex-patterns', 'cohort'],
    stages: [4, 5],
  },
  ...
  },
  {
    ...
    category: 'pattern',
    relatedTerms: ['simple-patterns', 'complex-patterns', 'fuzzy-patterns', 'architecture-composition'],
    stages: [0, 4, 5, 6],
  },
  {
    term: 'pattern-router',
    ...
    fullDefinition: 'In LLM-based solutions, not all inputs should be processed the same way. A pattern router examines each incoming item, identifies which pattern cluster it belongs to (simple, complex, or fuzzy), and sends it to the pipeline component optimised for that pattern. For customer support, this means billing disputes (simple pattern) get direct classification while compound multi-issue tickets (fuzzy pattern) get retrieval-augmented processing with human oversight.',
    ...
    category: 'model-component',
    relatedTerms: ['architecture-composition', 'simple-patterns', 'fuzzy-patterns'],
    stages: [0],
  },
  {
  ...
  },
  {
    ...
    category: 'metric',
    relatedTerms: ['pattern-router', 'simple-patterns', 'fuzzy-patterns'],
    stages: [0],
  },
  {
    term: 'sentiment-gate',
    ...
    fullDefinition: "A sentiment gate analyses the emotional tone of incoming text (anger, frustration, urgency) and uses that signal to influence routing decisions. In customer support, a ticket expressing extreme anger triggers the sentiment gate, which routes it to a human-reviewed escalation path rather than an automated response — even if the underlying issue (shipping delay) is a simple pattern that could otherwise be auto-classified. VibeModel composes sentiment gates when fuzzy or emotionally-charged patterns are detected in your ticket data.",
  ...
  },
  {
    ...
    category: 'pattern',
    relatedTerms: ['simple-patterns', 'complex-patterns', 'fuzzy-patterns', 'pattern-router'],
    stages: [0],
  },
]
...
export const stageExplainers: StageExplainer[] = [
  ...
  },
  {
    ...
    businessExplanation: 'We\'ve grouped your data into segments that behave similarly. Some have strong, straightforward signals that the agent can immediately recognise and handle (Simple), some require the agent to synthesise multiple features or cross-reference before confidently routing (Complex), and some are ambiguous — the agent sees similar signals across different outcomes, likely because a differentiating real-world parameter isn\'t captured in the dataset yet (Fuzzy).',
    analogy: 'Like sorting mail: some clearly go to one address and can be handled immediately (Simple), some require you to check multiple clues on the envelope (Complex), and some look identical on the outside — you\'d need to open them or get more context to know where they belong (Fuzzy).',
  },
  {
    stageId: 5,
