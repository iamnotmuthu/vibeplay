...
const FAQ_ARCHITECTURE: ArchitectureStageData = {
  ...
  lanes: [
    lane(1, 'Handled Automatically', '#10b981', 'autonomous',
      'Direct retrieval questions flow through a simple RAG pipeline. Questions come in, knowledge gets retrieved, responses go out. No reasoning chains needed.',
      112,
      [
        ...
        comp('auth', 'Authentication', 'ingestion', 'Validates caller identity', 2, 'JWT validation ensures only authorized users access the KB'),
        comp('input-validation', 'Input Validation', 'ingestion', 'Sanitizes the query', 3, 'Prevents malformed queries from reaching retrieval'),
        comp('request-classification', 'Request Classification', 'routing', 'Identifies query as KB lookup', 4, 'Simple intent classification — is this a direct lookup or not?'),
        comp('rag', 'RAG Pipeline', 'context', 'Retrieves matching documents', 5, 'Embedding-based search against knowledge base', ['request-classification'], { chunkSize: '512 tokens', topK: '3' }),
...
const DOC_INTEL_ARCHITECTURE: ArchitectureStageData = {
  ...
  lanes: [
    lane(1, 'Handled Automatically', '#10b981', 'autonomous',
      'Structured document extraction flows through a tool-augmented pipeline. Tables, single fields, and multi-section summaries are parsed without human input.',
      156,
      [
        comp('api-gateway', 'API Gateway', 'ingestion', 'Entry point with file upload handling', 1, 'Supports multipart uploads up to 50MB'),
        comp('auth', 'Authentication', 'ingestion', 'Validates access permissions', 2, 'Document-level access control enforcement'),
        comp('input-validation', 'Input Validation', 'ingestion', 'Validates file format and query', 3, 'Rejects unsupported formats early'),
        comp('pii-security', 'PII / Security Scanner', 'ingestion', 'Scans uploaded documents for sensitive data', 4, 'Masks SSN, account numbers before processing'),
        comp('request-classification', 'Request Classification', 'routing', 'Identifies extraction type', 5, 'Table vs. field vs. summary extraction'),
        comp('tool-planning', 'Tool Planning', 'execution', 'Selects extraction strategy', 6, 'Chooses parser chain based on document format', ['request-classification'], { strategy: 'format-adaptive' }),
        comp('tool-execution', 'Tool Execution', 'execution', 'Runs extraction pipeline', 7, 'OCR + table detection + field mapping', ['tool-planning']),
...
const RESEARCH_ARCHITECTURE: ArchitectureStageData = {
  ...
  lanes: [
    lane(1, 'Handled Automatically', '#10b981', 'autonomous',
      'Single-vendor lookups and structured comparisons flow through an API-augmented retrieval pipeline. Feature matrices and filtered searches resolve without reasoning chains.',
      178,
      [
        comp('api-gateway', 'API Gateway', 'ingestion', 'Entry point for research queries', 1),
        comp('auth', 'Authentication', 'ingestion', 'Validates API credentials', 2),
        comp('input-validation', 'Input Validation', 'ingestion', 'Validates query structure', 3),
        comp('prompt-injection', 'Prompt Injection Detection', 'ingestion', 'Screens research queries', 4, 'Prevents injection via crafted vendor names'),
        comp('request-classification', 'Request Classification', 'routing', 'Identifies research query type', 5, 'Single-vendor lookup vs. comparison vs. ranking'),
        comp('rag', 'RAG Pipeline', 'context', 'Retrieves vendor data from knowledge base', 6, 'Vendor profiles, pricing, certifications', ['request-classification']),
    ...
    lane(2, 'Handled with Reasoning', '#f59e0b', 'supervised',
      ...
      [
        ...
        comp('rag', 'RAG Pipeline', 'context', 'Knowledge base retrieval', 7, 'Internal vendor profiles and historical data', ['workflow-orchestrator']),
...
const DENTAL_ARCHITECTURE: ArchitectureStageData = {
  ...
  lanes: [
    lane(1, 'Handled Automatically', '#10b981', 'autonomous',
      'Scheduling, policy lookups, insurance panel checks, and reminders resolve through direct retrieval and API calls. No clinical judgment required.',
      198,
      [
        ...
        comp('request-classification', 'Request Classification', 'routing', 'Identifies request type', 6, 'Scheduling vs. policy vs. insurance vs. clinical'),
        comp('rag', 'RAG Pipeline', 'context', 'Office policy retrieval', 7, 'Cancellation policy, hours, accepted insurance', ['request-classification']),
...
const CODING_AGENT_ARCHITECTURE: ArchitectureStageData = {
  ...
  lanes: [
    ...
    lane(2, 'Handled with Reasoning', '#f59e0b', 'supervised',
      'Complex refactoring, architectural changes, and feature implementations require reasoning about design patterns and cross-module impact. The agent proposes changes but flags areas needing human review.',
      68,
      [
        comp('api-gateway', 'API Gateway', 'ingestion', 'Entry point', 1),
  ...
  lanes: [
    ...
    lane(2, 'Handled with Reasoning', '#f59e0b', 'supervised',
      ...
      [
        ...
        comp('tool-planning', 'Tool Planning', 'execution', 'Plans cross-modal synthesis', 8, 'Maps from one modality to another', ['rag', 'api-integration']),
