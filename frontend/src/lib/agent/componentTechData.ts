...
export interface EvalMetric {
  ...
  baseline: string // pre-optimization baseline value
...
const TILE_EVAL_METRICS: Record<string, TileEvalMetrics> = {
  ...
  'onprem-assistant': {
    ...
    metric2: {
      name: 'Resource Efficiency (TPS/Watt)',
      shortName: 'TPS/Watt',
      description: 'Tokens per second per watt of power draw during inference on constrained hardware.',
      baseline: '8.1',
      actual: '14.2',
      target: '> 12',
      unit: 'TPS/W',
  ...
  'multimodal-agent': {
    ...
    metric2: {
      name: 'Semantic Alignment (CLIP Score)',
      shortName: 'CLIP Score',
      description: 'CLIP score measuring text-image semantic alignment across multimodal outputs.',
      baseline: '0.67',
      actual: '0.82',
      target: '> 0.78',
      unit: 'CLIP',
